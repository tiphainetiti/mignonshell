...
/*   Updated: 2025/04/02 14:45:01 by tlay             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*update_buffer(int fd, char *buffer, char *remaining_buffer)
{
	...
	while (bytes_read > 0)
	{
		...
		buffer[bytes_read] = '\0';
		if (!remaining_buffer)
			remaining_buffer = ft_strdup_gnl("");
		tmp = remaining_buffer;
		remaining_buffer = ft_strjoin_gnl(tmp, buffer);
		free(tmp);
		tmp = NULL;
...
char	*get_line(char *actual_buffer)
{
	...
	if (actual_buffer[eol] == '\n')
		eol++;
	line = ft_substr_gnl(actual_buffer, i, eol);
	return (line);
}
...
static char	*substract_of_buffer(char *actual_buffer, char *line)
{
	...
	eob = ft_strlen_gnl(actual_buffer);
	eol = ft_strlen_gnl(line);
	remaining_buffer = ft_substr_gnl(actual_buffer, eol, eob);
	if (!remaining_buffer)
